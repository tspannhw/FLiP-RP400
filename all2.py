...
try:
    ...
    while True:
        for fontname, size in [(None, None), ("tiny.ttf", 6), ("ProggyTiny.ttf", 16), ("creep.bdf", 16), ("miscfs_.ttf", 12), ("FreePixel.ttf", 12), ('ChiKareGo.ttf', 16)]:
            font = ImageFont.load_default() # make_font(fontname, size) if fontname else None   # 
            term = terminal(screendevice, font)
            ...
            print(rp400Record)
            term.println("rp400-apachepulsar")
            term.println("------------------")
            term.println(str(ipaddress))
            term.println(str(round(gas.read_all().oxidising,2)))
            term.println(str(starttime))
